...
void push_back(EmployeeVector *vector, Employee newEmployee);
void push_front(EmployeeVector *vector, Employee newEmployee);
...
void generate_random_data(EmployeeVector *vector, int number_of_employees)
{
    for (int i = 0; i < number_of_employees; i++)
    {
        ...
        sprintf(new_employee.name, "Employee%d", i + 1);
...
void read_employee_details(EmployeeVector *vector)
{
    ...
    print_employee_details(&vector->data[employee_number]);
}
